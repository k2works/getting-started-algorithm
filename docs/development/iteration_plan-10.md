...
- [x] 各章のコード例が `apps/fsharp/` の実コードと同期している（記事記述と実装を同一コミットで完結）
- [x] `apps/fsharp/` のテストが全てパス（`dotnet test`）
- [x] mkdocs.yml の nav に F# 版全 9 章が追加されている
- [x] ローカルプレビューで表示確認済み（`mkdocs build` でビルド成功）
- [x] 各章執筆時に Python 版との内容差分チェックを実施済み（差分 30% 以内）
- [x] F# の関数型スタイル（パイプライン演算子、パターンマッチ、`option<T>`）を活用した実装
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 1-1 | F# プロジェクト作成（apps/fsharp/、getting-started-algorithm.sln） | 20 分 | [x] |
| 1-2 | 章ごとのプロジェクト分割（Chapter01〜Chapter09 + Tests） | 20 分 | [x] |
| 1-3 | `.gitignore` 作成（`bin/`、`obj/`、`.vs/`） | 5 分 | [x] |
| 1-4 | Nix devShell（.#dotnet）設定確認 | 15 分 | [x] |

### 2. CI 整備（0.5 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 2-1 | CI 設定（.github/workflows/ci-fsharp.yml: `dotnet test`） | 20 分 | [x] |
| 2-2 | CI 動作確認 | 10 分 | [x] |
...
| 3-2 | 記事執筆（docs/article/fsharp/01-basic-algorithms.md） | 30 分 | [x] |
...
| 4-2 | 記事執筆（docs/article/fsharp/02-arrays.md） | 20 分 | [x] |
...
| 5-2 | 記事執筆（docs/article/fsharp/03-search-algorithms.md） | 20 分 | [x] |
...
| 6-2 | 記事執筆（docs/article/fsharp/04-stacks-and-queues.md） | 20 分 | [x] |
...
| 7-2 | 記事執筆（docs/article/fsharp/05-recursion.md） | 20 分 | [x] |
...
| 8-2 | 記事執筆（docs/article/fsharp/06-sort-algorithms.md） | 20 分 | [x] |
...
| 9-2 | 記事執筆（docs/article/fsharp/07-strings.md） | 20 分 | [x] |
...
| 10-2 | 記事執筆（docs/article/fsharp/08-linked-lists.md） | 20 分 | [x] |
...
| 11-2 | 記事執筆（docs/article/fsharp/09-trees.md） | 20 分 | [x] |

### 12. ドキュメント整備（0.5 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 12-1 | index.md 作成（docs/article/fsharp/index.md） | 30 分 | [x] |
| 12-2 | mkdocs.yml に F# 版 9 章を追加 | 10 分 | [x] |
...
- [x] `apps/fsharp/` の全テストがパス（`dotnet test`）— 238 テスト全合格
- [x] 全 9 章 + index.md が作成されている
- [x] mkdocs.yml の nav に F# 版全 9 章が追加されている
- [x] ローカルプレビューで表示確認済み（`npm run docs:build` でビルド成功）
...
|------|---------|--------|
| 2026-04-12 | 初版作成 | - |
| 2026-04-13 | 全タスク完了（238 テスト全合格、mkdocs ビルド確認） | - |
...
- F# の型システム（`[<AllowNullLiteral>]`、型制約）を活用した安全な実装ができた
- Python 版との差分補完により、概念説明・計算量表を充実化できた
- apps/csharp と同じ 2 プロジェクト構成（Algorithm/ + Algorithm.Tests/）への再編により、プロジェクト構造の一貫性が向上した
- CI ワークフロー（ci-fsharp.yml）の設定がスムーズに完了した
- IT-9 のふりかえりで得た教訓（`.gitignore` の事前準備、ローカルプレビュー確認の明示化、記事記述量チェック）を反映できた

### 改善が必要なこと

- F# の制約（例外処理・ネスト型のモジュールレベル制限）への対応に試行錯誤が必要だった
- 記事の充実化は Python 版の約 30% の分量にとどまった（概念説明・計算量表の追記は実施したが、さらなる深掘りの余地がある）
- イテレーション計画のディレクトリ構成が実際の再編後の構成と異なっていた（Chapter ベース → Algorithm/Algorithm.Tests ベースへの変更）

### 次のイテレーションへの教訓

- 次の言語（Scala）でも同様の 2 プロジェクト構成（メインプロジェクト + テストプロジェクト）を最初から採用する
- 記事作成時は概念説明セクションを最初から含める計画にする
- ディレクトリ構成の設計は、同一エコシステムの既存言語（C#/.NET）に合わせることで再編の手戻りを防ぐ
- 型制約やモジュール構成など言語固有の制約は、実装前に調査・検証時間を見積もりに含める
