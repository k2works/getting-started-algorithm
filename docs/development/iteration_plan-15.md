...
- [ ] 統合解説の全章ファイルが作成されている
- [ ] 各章に 14 言語の実装比較（コード例 + 比較表）が含まれている
- [ ] 言語パラダイム別の設計思想比較が記述されている
- [ ] mkdocs.yml の nav に多言語統合解説が追加されている
- [ ] ローカルプレビューで表示確認済み（`npx gulp mkdocs:build` でビルド成功）
- [ ] `docs/article/index.md` の多言語統合解説リンクが有効
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 1-1 | `docs/article/all/` ディレクトリ作成 | 5 分 | [ ] |
| 1-2 | `docs/article/all/index.md` 作成（目次・導入・言語一覧・読み方ガイド） | 30 分 | [ ] |
| 1-3 | 14 言語の分類整理（パラダイム・型システム・メモリ管理・主要特徴） | 25 分 | [ ] |

### 2. 第 1 章 言語概要と開発環境比較（1.0 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 2-1 | `docs/article/all/01-language-overview.md` 執筆 | 60 分 | [ ] |
| 2-2 | 言語パラダイム分類表（OOP / 手続き型 / 関数型 / 純粋関数型）作成 | 30 分 | [ ] |
| 2-3 | 型システム比較表（静的/動的・強い/弱い・型推論）作成 | 20 分 | [ ] |
| 2-4 | 開発環境・テストフレームワーク比較表作成 | 20 分 | [ ] |

### 3. 第 2 章 基本構文と制御フロー比較（1.0 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 3-1 | `docs/article/all/02-basic-syntax-comparison.md` 執筆 | 60 分 | [ ] |
| 3-2 | 変数宣言・条件分岐・ループの 14 言語コード比較 | 30 分 | [ ] |
| 3-3 | `max3`・`mid3` の 14 言語実装比較表作成 | 30 分 | [ ] |

### 4. 第 3 章 配列・コレクション比較（1.0 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 4-1 | `docs/article/all/03-arrays-and-collections.md` 執筆 | 60 分 | [ ] |
| 4-2 | 配列/リスト/スライスの言語別データ構造比較表作成 | 30 分 | [ ] |
| 4-3 | 探索アルゴリズム（線形探索・二分探索）の実装パターン比較 | 30 分 | [ ] |

### 5. 第 4 章 データ構造の実装アプローチ比較（1.0 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 5-1 | `docs/article/all/04-data-structures.md` 執筆 | 60 分 | [ ] |
| 5-2 | スタック・キューの実装パターン比較（クラス / 構造体 / モジュール / 代数的データ型） | 30 分 | [ ] |
| 5-3 | ハッシュ法の実装パターン比較 | 20 分 | [ ] |
| 5-4 | 可変/不変データ構造のアプローチ比較表 | 20 分 | [ ] |

### 6. 第 5 章 再帰とソートアルゴリズム比較（1.0 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 6-1 | `docs/article/all/05-recursion-and-sorting.md` 執筆 | 60 分 | [ ] |
| 6-2 | 再帰の表現力比較（末尾再帰最適化・パターンマッチ・ガード） | 30 分 | [ ] |
| 6-3 | ソートアルゴリズムの実装スタイル比較（破壊的/非破壊的） | 30 分 | [ ] |

### 7. 第 6 章 文字列処理と高度なデータ構造比較（1.0 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 7-1 | `docs/article/all/06-strings-and-advanced-structures.md` 執筆 | 60 分 | [ ] |
| 7-2 | 文字列探索（BF/KMP/BM）の実装スタイル比較 | 30 分 | [ ] |
| 7-3 | リスト・木構造の実装パターン比較（ポインタ / 参照 / 代数的データ型 / IORef） | 30 分 | [ ] |

### 8. 第 7 章 総合比較とまとめ（1.0 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 8-1 | `docs/article/all/07-comprehensive-comparison.md` 執筆 | 60 分 | [ ] |
| 8-2 | 言語選択ガイド（用途別おすすめ言語表）作成 | 30 分 | [ ] |
| 8-3 | パフォーマンス・可読性・安全性の総合評価表作成 | 30 分 | [ ] |
| 8-4 | 学習ロードマップ（どの順序で言語を学ぶべきか）作成 | 20 分 | [ ] |

### 9. mkdocs 統合と公開（0.5 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 9-1 | `mkdocs.yml` nav 更新（多言語統合解説セクション追加） | 15 分 | [ ] |
| 9-2 | `docs/article/index.md` の多言語統合解説リンク確認 | 10 分 | [ ] |
| 9-3 | `npx gulp mkdocs:build` でビルド確認 | 10 分 | [ ] |

### タスク合計

| カテゴリ | SP | 理想時間 | 状態 |
|---------|----|----------|------|
| 構成設計と index 作成 | 0.5 | 60 分 | [ ] |
| 第 1 章 言語概要と開発環境比較 | 1.0 | 130 分 | [ ] |
| 第 2 章 基本構文と制御フロー比較 | 1.0 | 120 分 | [ ] |
| 第 3 章 配列・コレクション比較 | 1.0 | 120 分 | [ ] |
| 第 4 章 データ構造の実装アプローチ比較 | 1.0 | 130 分 | [ ] |
| 第 5 章 再帰とソートアルゴリズム比較 | 1.0 | 120 分 | [ ] |
| 第 6 章 文字列処理と高度なデータ構造比較 | 1.0 | 120 分 | [ ] |
| 第 7 章 総合比較とまとめ | 1.0 | 140 分 | [ ] |
| mkdocs 統合と公開 | 0.5 | 35 分 | [ ] |
| **合計** | **8** | **約 975 分** | |
...
- [ ] 統合解説の全 7 章 + index.md が作成されている
- [ ] 各章に 14 言語の比較表が含まれている
- [ ] `npx gulp mkdocs:build` でビルド成功
- [ ] mkdocs.yml の nav に多言語統合解説が追加済み
- [ ] `docs/article/index.md` のリンクが有効
- [ ] ドキュメント更新完了（development/index.md 含む）
