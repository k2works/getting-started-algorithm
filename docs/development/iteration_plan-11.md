...
- [ ] 9 章すべてのファイルが作成されている
- [ ] 各章のコード例が `apps/scala/` の実コードと同期している（記事記述と実装を同一コミットで完結）
- [ ] `apps/scala/` のテストが全てパス（`sbt test`）
- [ ] mkdocs.yml の nav に Scala 版全 9 章が追加されている
- [ ] ローカルプレビューで表示確認済み（`mkdocs build` でビルド成功）
- [ ] 各章執筆時に Python 版との内容差分チェックを実施済み（差分 30% 以内）
- [ ] Scala の関数型スタイル（`Option[T]`、`case class`、パターンマッチ）を活用した実装
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 2-1 | CI 設定（.github/workflows/ci-scala.yml: `sbt test`） | 20 分 | [ ] |
| 2-2 | CI 動作確認 | 10 分 | [ ] |
...
| 3-2 | 記事執筆（docs/article/scala/01-basic-algorithms.md） | 30 分 | [ ] |
...
| 4-2 | 記事執筆（docs/article/scala/02-arrays.md） | 20 分 | [ ] |
...
| 5-2 | 記事執筆（docs/article/scala/03-search-algorithms.md） | 20 分 | [ ] |
...
| 6-2 | 記事執筆（docs/article/scala/04-stacks-and-queues.md） | 20 分 | [ ] |
...
| 7-2 | 記事執筆（docs/article/scala/05-recursion.md） | 20 分 | [ ] |
...
| 8-2 | 記事執筆（docs/article/scala/06-sort-algorithms.md） | 20 分 | [ ] |
...
| 9-2 | 記事執筆（docs/article/scala/07-strings.md） | 20 分 | [ ] |
...
| 10-2 | 記事執筆（docs/article/scala/08-linked-lists.md） | 20 分 | [ ] |
...
| 11-2 | 記事執筆（docs/article/scala/09-trees.md） | 20 分 | [ ] |

### 12. ドキュメント整備（0.5 SP）

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 12-1 | index.md 作成（docs/article/scala/index.md） | 30 分 | [ ] |
| 12-2 | mkdocs.yml に Scala 版 9 章を追加 | 10 分 | [ ] |
| 12-3 | ローカルプレビュー確認（`mkdocs build`） | 10 分 | [ ] |
