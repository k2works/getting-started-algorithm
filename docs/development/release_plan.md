...
| **目的** | アルゴリズムとデータ構造を 12 言語で TDD しながら学べる記事シリーズの執筆 |
...
Wiki 記事のうち最も完成度の高い Python 版を原本として作成し、Python 版を基に 11 言語へ多言語展開する。

| フェーズ | 内容 | 言語数 | ストーリー数 |
|---------|------|--------|-------------|
| Phase 1 | Python 原本作成 + OOP 言語展開 | 6 | 6 |
| Phase 2 | システム言語 + 関数型言語展開 | 6 | 6 |
| Phase 3 | 多言語統合解説 | - | 1 |
| **合計** | | **12 + 統合** | **13** |
...
### Phase 3: 多言語統合解説（イテレーション 13）

| ID | ユーザーストーリー | SP | BV | C | KA | RR | 優先度 |
|----|-------------------|----|----|---|----|----|--------|
| US-013 | 多言語統合解説（12 言語横断比較）を執筆する | 8 | 高 | 高 | 高 | 低 | 中 |
| **合計** | | **8** | | | | | |
...
gantt
    ...
    Release 0.2.0             :milestone, m2, after p2-6, 0d
    section Phase 3 統合解説
    IT13 多言語統合解説        :p3-1, after p2-6, 14d
...
- **全体バッファ**: 15%（13 + 2 = 15 イテレーション = 30 週間）
...
**ゴール**: システム言語・関数型言語（Go, Rust, F#, Scala, Clojure, Haskell）を Python 版から順次展開

**目標 SP**: Go 3 SP、他 各 5 SP

### イテレーション 13（Week 25-26）
...
xychart-beta
    ...
    y-axis "残 SP" 0 --> 60
    line "計画" [56, 51, 48, 45, 42, 39, 36, 33, 28, 23, 18, 13, 8, 0]
```
